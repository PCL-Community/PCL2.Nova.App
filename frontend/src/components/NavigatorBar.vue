        ...
        <Transition name="fade" mode="out-in">
            <section class="flex justify-between" v-if="$route.meta['navbar_mode'] === 'normal'">
                <section class="absolute top-1/2 left-1/2 -translate-1/2 flex">
                    <MenuButton text="启动" :selected="$route.fullPath === '/'" @click="$router.push('/')">
                        <i class="icon-[material-symbols--play-arrow-outline] size-8 -ml-2"></i>
                    </MenuButton>
                    <MenuButton
                        text="下载"
                        :selected="$route.fullPath.startsWith('/download')"
                        @click="$router.push('/download')">
                        <i class="icon-[material-symbols--download-sharp] size-7 -ml-1"></i>
                    </MenuButton>
                    <MenuButton
                        text="设置"
                ...
                <section class="flex items-center">
                    <IconButton rounded flex-center @click="WindowMinimise">
                        <i class="icon-[ic--round-minus] size-8 bg-white"></i>
                    </IconButton>
                    <IconButton rounded flex-center class="-ml-1">
                        <i class="icon-[material-symbols--close] size-8 bg-white" @click="Quit"></i>
                    </IconButton>
                </section>
            </section>
        </Transition>
