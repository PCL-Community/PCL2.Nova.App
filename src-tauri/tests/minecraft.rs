...
fn version_list_get_test(){
    let res = tokio::runtime::Runtime::new().unwrap().block_on(minecraft::download::vanilla::VersionManifestOverall::new());
    match res {
        Ok(v) => println!("{:?}", v),
    ...
    tokio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        match res {
            Ok(v) => {
                ...
                latest.download(dirs_next::desktop_dir().unwrap()).await.unwrap();
            }
            Err(e) => println!("Error: {}", e)
        }
    });
